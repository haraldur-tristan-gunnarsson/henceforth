...
#define NATIVE_WORD(CNAME,NAME,NEXT) void CNAME##_native(void); /*FORWARD DECLARATION*/ \
	...
	void CNAME##_native(void) // Use: NATIVE_WORD(CNAME,NAME,NEXT) { ...code... }
...
#define THREADED_ENTRY(CNAME,NAME,NEXT) \
	const struct entry CNAME##_entry ={ \
		...
		.can_delete = 0, \
	};//After definition, as cannot use 'sizeof' with forward declaration.
//}
...
NATIVE_WORD(exit,"exit",null){
	exit(0);
}

NATIVE_WORD(reset_stack,"reset_stack",exit){
	data_ptr = (size_t*)data_stack;
}

NATIVE_WORD(align_code,"ALIGN_CODE",reset_stack){//Used to make dumps easier to read.
...
NATIVE_WORD(show_data,".s",align_code){//Display contents of data stack.
	size_t *ii;
	printf("data: ");
...
NATIVE_WORD(show_proc,".r",show_data){//Display contents of return stack.
	size_t *ii;
	printf("proc: ");
...
#define VARIABLE(CNAME,NAME,NEXT) NATIVE_WORD(CNAME,NAME,NEXT){ \
	push_data((size_t)(&(CNAME))); \
}//( -- system_variable ) Read 'var @'. Write 'val var !'.

#define BINARY_OP(CNAME,OP,NAME,NEXT) NATIVE_WORD(CNAME,NAME,NEXT){ \
	size_t val2 = pop_data(); \
	size_t val1 = pop_data(); \
	push_data(val1 OP val2); \
}//( n1 n2 -- n1OPn2 ) For C-derived binary operation primitives.

NATIVE_WORD(not, "NOT", show_proc){//( n -- notn )
	push_data(pop_data() ? 0 : ~0);// ~0 (-1) is TRUE, 0 is FALSE
}

#define BOOLEAN_OP(CNAME,OP,NAME,NEXT) NATIVE_WORD(CNAME,NAME,NEXT){ \
	size_t val2 = pop_data(); \
	size_t val1 = pop_data(); \
	push_data(val1 OP val2); \
	not_native(); \
	not_native(); \
}//( n1 n2 -- n1OPn2 ) For C-derived boolean operations.
...
NATIVE_WORD(bnot,"BNOT",band){//( n -- ~n )
	push_data(~pop_data());
}
...
NATIVE_WORD(literal,"LITERAL",global_in){//Get next code value and skip it. Needed for numerical constants.
	push_data(*inst_ptr++);//( -- n )
}

NATIVE_WORD(ret,"RET",literal){//Return from called word to calling (virtual) word
#ifdef DEBUG
	show_data_native();
	show_proc_native();
	printf("%d %s\n", __LINE__, __FILE__);
...
NATIVE_WORD(stdin,"STDIN",cell){//( -- page_size )
	push_data((size_t)stdin);//CONSTANT macro, like HERE, CELL?
}

NATIVE_WORD(pagesize,"PAGESIZE",stdin){//( -- page_size )
	push_data(page_size);//CONSTANT macro, like HERE, CELL?
}


NATIVE_WORD(cin,"CIN",pagesize){//( - c - ) Get next character from input stream.
	push_data((size_t)getc(global_in));
}

NATIVE_WORD(at,"@",cin){//( addr -- n ) size_t pointer dereference for top of data stack.
	push_data(*((size_t *)pop_data()));
}

NATIVE_WORD(set_value,"!",at){//( n addr -- ) Set size_t pointed to by top of data stack.
	size_t *ref = (size_t *)pop_data();
	size_t val = pop_data();
	*ref = val;
}

NATIVE_WORD(char_at,"c@",set_value){//( addr -- n ) Like @/at, but for a single char/byte.
	push_data((size_t)(*((char *)pop_data())));
}

NATIVE_WORD(char_set_value,"c!",char_at){//( n addr -- ) Like !/set_value, but for a single char/byte.
	char *ref = (char *)pop_data();
	char val = (char)pop_data();
	*ref = val;
}

NATIVE_WORD(m_set_end,"M_SET_END",char_set_value){//( addr -- ) Handles virtual memory for the 'code space'.
...
NATIVE_WORD(drop,"DROP",m_set_end){//( n -- )
	(void)pop_data();
}

NATIVE_WORD(rdrop,"RDROP",drop){//( r: addr -- )
	(void)pop_proc();
}

NATIVE_WORD(swap,"SWAP",rdrop){//( n1 n2 -- n2 n1 )
	size_t val2 = pop_data();
	size_t val1 = pop_data();
	push_data(val2);
	push_data(val1);
...
NATIVE_WORD(s_to_r,">r",swap){//( n -- ) ( r: -- n )
	push_proc(pop_data());
}

NATIVE_WORD(r_to_s,"r>",s_to_r){//( r: addr -- ) ( -- addr )
	push_data(pop_proc());
}

NATIVE_WORD(s_to_r_copy,"@r",r_to_s){//( n -- n ) ( r: -- n )
	push_proc(read_data());
}

NATIVE_WORD(r_to_s_copy,"r@",s_to_r_copy){//( r: addr -- addr ) ( -- addr )
	push_data(read_proc());
}

NATIVE_WORD(here,"HERE",r_to_s_copy){// ( -- addr ) CPTR @
	code_ptr_native();
	at_native();
}

NATIVE_WORD(decprint,"(.)",here){//( n -- )
	printf("%zd",pop_data());
}

NATIVE_WORD(hexprint,"(x.)",decprint){//( n -- )
	printf("%zx",pop_data());
}

NATIVE_WORD(emit, "emit", hexprint){//( c -- )
	putchar((char)pop_data());
}

NATIVE_WORD(trace,"TRACE",emit){//Display contents of return stack as 'word' names for debugging.
...
NATIVE_WORD(call,"CALL",trace){// ( xt -- ) given execution token (xt, entry) execute its code
...
NATIVE_WORD(number,"#",call){// ( char* -- n ) Converts a null-terminated string to an integer.
	char *test = "";
...
NATIVE_WORD(bsw, "BSW", number){//( -- "word" ) BSW: blank-separated word, null-terminated.
	int word_index = 0;
...
NATIVE_WORD(nprint, "(n.)", bsw){//( "string" -- ) print null-terminated string.
	fputs((char *)pop_data(), stdout);
}

NATIVE_WORD(wordsearch, "WORDSEARCH", nprint){//( "word" -- xt ) Find (latest, most recent) xt for given word.
	char *word = (char *) pop_data();
	struct entry *current = head;
	while(current){
...
NATIVE_WORD(exec, "EXEC", wordsearch){//( "word" -- | n ) Run given word (i.e. null-terminated string).
...
NATIVE_WORD(quote, "'", exec){//( - "word" - xt ) Get subsequent word's xt.
	bsw_native();		//~ BSW
...
NATIVE_WORD(xaddr, "XADDR", quote){//( xt -- xaddr ) Get execution address of threaded code.
...
NATIVE_WORD(name_out, "NAME_OUT", xaddr){//( xt -- ) Print out the xt's name or, failing that, its value
...
NATIVE_WORD(seext,"seext",name_out){//( xt -- ) Print out the xt's code length and token string (code).
	struct entry *xt = (struct entry*)pop_data();
...
NATIVE_WORD(xname,"XNAME",seext){//( xt -- "name" )
	struct entry *xt = (struct entry*)pop_data();
	push_data((size_t)xt->name);
}

NATIVE_WORD(words,"WORDS",xname){//Print out the names of all words. Many definitions --> words repeat.
	struct entry *current = head;
	while(current){
...
NATIVE_WORD(prompt,"PROMPT",words){//Print out a nice (?) prompt for interactive use. Imperfect.
...
NATIVE_WORD(outer_interpreter, "OUTER_INTERPRETER", prompt){//This is what the user interacts with.
...
NATIVE_WORD(allot,"ALLOT",outer_interpreter){//( n -- ) Allocate given amount of bytes of memory to 'code space'.
	here_native();			//~ HERE
	add_native();			//~ +
...
NATIVE_WORD(cstrcmp, "CSTRCMP", compile){//( s1 s2 -- n ) Forthy interface for C's strcmp().
	char *s2 = (char *)pop_data();
	char *s1 = (char *)pop_data();
	push_data((size_t)strcmp(s1, s2));
}

NATIVE_WORD(start_threaded_code, "[", cstrcmp){//Until delimiter ("]") is reached, record into threaded code the execution tokens of given words or (for numbers) LITERAL plus the value.
...
NATIVE_WORD(colon, ":", start_threaded_code){//( - "name" - ) Create a temporary entry. Made permanent by ;.
...
NATIVE_WORD(semicolon, ";", colon){//Make all currently temporary dictionary entries permanent.
...
NATIVE_WORD(skip_bl, "SKIPBL", semicolon){//( c -- ) Given delimiter, skip input to space-delimited delimiter.
...
NATIVE_WORD(line_comment, "\\", skip_bl){
	int in;
	while(cin_native(),(in = (int)pop_data()) != EOF){
		if('\n' == in)break;
	}
